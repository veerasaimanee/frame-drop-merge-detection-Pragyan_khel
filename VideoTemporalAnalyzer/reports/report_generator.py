...
class ReportGenerator:
    ...
    def _generate_anomaly_snapshots(self, df, video_path, output_dir):
        ...
        for idx, row in flagged.iterrows():
            ...
            overlay = frame.copy()
            h, w = frame.shape[:2]
            cv2.rectangle(overlay, (0, 0), (min(w, 480), 160), (13, 17, 23), -1)
    ...
    def _generate_analysis_log(self, df, summary, output_dir):
        ...
        log_content = [
            ...
            f"Drop Threshold: deviation >= 1.6 AND signal confirmation",
            f"Merge Threshold: Balanced Two-Layer + AI Blend Detection",
            "-" * 50,
            ...
            "-" * 50,
            "DETECTION LOG (First 50):"
        ]
        
        anomalies = df[df['status'] != 'NORMAL'].head(50)
        for _, row in anomalies.iterrows():
            log_content.append(f"• Frame {int(row['frame_index']):04d}: {row['status']} | Dev: {row['temporal_deviation']:.2f}x | Sz: {row['ssim_z']:.2f} | Lz: {row['laplacian_z']:.2f}")
    ...
    def _generate_pdf(self, summary, results_df, output_dir, flagged_images):
        ...
        pdf.cell(0, 5, "Strict Forensic Temporal Integrity Validator v3.0", ln=True, align='C')
        pdf.ln(10)
        ...
        # Methodology
        pdf.set_font("Helvetica", "B", 14)
        pdf.cell(0, 10, "2. Forensic Methodology (v3.0)", ln=True)
        pdf.set_font("Helvetica", "", 10)
        methodology = (
            "This system employs a deterministic rule-based engine using a Global Robust Statistical Model. "
            "Unlike rolling-window systems, v3.0 uses the entire clip distribution to calculate Median Absolute Deviation (MAD) "
            "Z-scores, ensuring stability for clips ranging from 0.1s to 10s. "
            "DROPS are identified via temporal deviation (>= 1.6x) with structural cross-verification. "
            "MERGES are identified by convergent SSIM/Laplacian drops or AI-blend signatures (high similarity + zero motion)."
        )
        ...
        pdf.cell(0, 8, f"Measured Stream FPS: {summary['computed_fps']:.2f}", ln=True)
        ...
        if flagged_images:
            ...
            for img in flagged_images[:10]: # Show up to 10 in PDF
